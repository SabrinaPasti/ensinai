...
app.get("/:id/:idcurso/aulas", async (req, res) => {
  ...
  try {
    ...
    const alunoInscritoNoCurso = alunoCurso.some(
      (ac) => ac.id_curso === id_curso
    );
...
app.get("/:id/:idcurso/:idaula/interna-aula", async (req, res) => {
  ...
  try {
    ...
    const arquivoUrl = aula.arquivo
      ? `${supabaseUrl}/storage/v1/object/public/ARQUIVOS/${aula.arquivo}`
      : null;
...
app.post("/concluir-aula/:idAula", async (req, res) => {
  ...
  try {
    console.log(
      `Recebido tempo de ${tempoGasto} minutos para a aula ${idAula}`
    );
...
app.get("/:id/historico/", async (req, res) => {
  try {
    ...
    const pendentes = atividadesComUrls.filter(
      (atv) => atv.status === "pendente"
    );
    const vencidas = atividadesComUrls.filter(
      (atv) => atv.status === "vencida"
    );
    const entregues = atividadesComUrls.filter(
      (atv) => atv.status === "entregue"
    );
